...
from handlers import delete_user
...
def test_delete_user_invalid_user_id():
    ...
    response = delete_user(mongo_client, event, context)
...
def test_delete_user_success():
    mongo_client = mongomock.MongoClient()
    # Insert the user so it can be deleted
    insert_result = mongo_client.users.users.insert_one({"_id": ObjectId()})
