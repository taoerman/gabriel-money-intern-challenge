...
def get_user_aggregated_info(mongo_client, event, context):
    ...
    user_response = requests.get(USER_URL.format(user_id=user_id))
    accounts_response = requests.get(ACCOUNTS_URL.format(user_id=user_id))
    cards_response = requests.get(CARDS_URL.format(user_id=user_id))
    transactions_response = requests.get(TRANSACTIONS_URL.format(user_id=user_id))
